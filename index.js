...
const commandLol = new SlashCommandBuilder()
  .setName("lol")
  .setDescription("Donnes tes ranks sur LoL");
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content.startsWith(process.env.PREFIX)) {
    ...
    switch (command) {
      case "team":
        ...
        for (let i = 0; i < channelSize; i++) {
          ...
          message.channel.send(
            "Le joueur " + memberTab[i] + " est " + roleValue
          );
      ...
      case "code":
        message.channel.send(
          "Pour partager du code, merci d'utiliser le lien suivant : **<@" +
            message.author.id +
            ">**"
        );
        break;
      ...
      case "prediction":
        ...
        var phrase = [
          "C'est définitivement lelo que tu mérites.",
          "Si tu y arrives, je mange mon chapeau.",
          "Même mon Colin peut y arriver, alors pourquoi pas toi ?",
          "Bon, je serais toi je demanderai à quelqu'un de te PL.",
        ...
        if (tierValue === "Master" || tierValue === "Challenger") {
          ...
          message.channel.send(
            "<@" +
              message.author.id +
              ">  Cette saison, je prédis que tu seras capable d'atteintre le **" +
              tierValue +
              " " +
              points +
              " " +
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "champion-random") {
    ...
    await cmd.execute(interaction);
  } else if (interaction.commandName === "team-builder") {
